...
/**
 * Verify CBE Bank Receipt Screenshot using OCR
 */
export const verifyBankReceipt = async (req, res) => {
    try {
        ...
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'Receipt screenshot is required' });
        }

        // 1. Upload to Cloudinary for persistence
        ...
        const extractedData = await OCRProcessor.processCBEReceipt(req.file.buffer);

        if (!extractedData.amount) {
            return res.status(422).json({
                success: false,
                message: 'Failed to extract payment amount. Please ensure the screenshot is clear.',
                extracted: extractedData
            });
        }

        // Generate Pseudo-Transaction ID if missing
        if (!extractedData.transactionID) {
            extractedData.transactionID = `CBE-AUTO-${crypto.createHash('md5').update(req.file.buffer).digest('hex').substring(0, 10).toUpperCase()}`;
        }
        ...
        if (!vehicle || !vehicle.ownerDetails) {
            return res.status(404).json({ success: false, message: 'Vehicle owner details not found for this trip' });
        }
        ...
        // Mark as suspicious if names don't match OR confidence is low (< 60%)
        ...
        // 5. Finalize Payment (Success or Under Review)
        ...
        if (paymentStatus === 'success') {
            ...
            booking.paymentStatus = 'pending'; // Stays pending until admin reviews
            await booking.save();
        }
        ...
        if (paymentStatus === 'success') {
            try {
                await NotificationService.createNotification({
                    ...
                    message: `Your CBE transfer of ETB ${extractedData.amount} has been verified. Booking confirmed!`,
                    type: 'payment_success',
                    priority: 'medium',
                    channel: 'all'
                });
            ...
            try {
                ...
                await NotificationService.createNotification({
                    ...
                    message: `We received your receipt, but it requires manual confirmation by our team. Please wait.`,
                    type: 'payment_pending',
                    priority: 'medium',
                    channel: 'all'
                });

                // Notify Station Admin (optional: simplified for now)
                console.log('⚠️ Manual Review Required for Payment:', payment._id);
            } catch (nErr) { console.error(nErr); }
        }
        ...
        res.status(200).json({
            ...
            message: isSuspicious
                ? 'Receipt uploaded. An admin must verify this manually due to low image quality or name mismatch.'
    ...
    try {
        ...
        res.status(200).json({
            ...
            data: {
                ...
                instructions: `Please transfer ETB ${booking.totalPrice || booking.tripID?.price} to the account above. After payment, take a screenshot of the receipt and upload it here to confirm your seat.`
            }
        });
    } catch (error) {
