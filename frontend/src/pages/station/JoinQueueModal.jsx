...
const JoinQueueModal = ({ isOpen, onClose, stationID, onSuccess }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            toast.error('Failed to load vehicles or routes');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                toast.success('Vehicle added to queue successfully');
                onSuccess();
                onClose();
            }
        } catch (error) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                    <h2 className="text-xl font-bold text-gray-800">Add Vehicle to Queue</h2>
                ...
                {loading ? (
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                                ...
                                >
                                    ...
                                    {vehicles.map(v => (
                                        <option key={v._id} value={v._id}>
                                            {v.plateNumber} ({v.carType}) - Driver: {v.driverID?.fullName || 'N/A'}
                                        </option>
                                    ))}
                                </select>
                                {vehicles.length === 0 && (
                                    <p className="text-xs text-amber-600 mt-1 italic">No available vehicles found at this station.</p>
                                )}
                            </div>

                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-2 flex items-center gap-2">
                                    <MapPin className="w-4 h-4" /> Select Route
                                </label>
                                <select
                                    value={selectedRoute}
                        ...
                        <div className="pt-4 flex gap-3">
                            ...
                            >
                                {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Join Queue'}
                            </button>
                        </div>
                    </form>
                )}
